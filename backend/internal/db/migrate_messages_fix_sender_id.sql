-- Repair the messages table for the newer code paths that expect sender + sender_id + sender_name + message_type.
...
-- Backfill legacy rows so the older sender field remains usable.
UPDATE messages
